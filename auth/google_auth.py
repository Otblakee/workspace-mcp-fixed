...
async def start_auth_flow(
    ...
    try:
        ...
        # Capture the PKCE code_verifier generated by authorization_url()
        # so we can restore it when handling the callback
        ...
        store = get_oauth21_session_store()
        store.store_oauth_state(oauth_state, session_id=session_id, code_verifier=code_verifier)
...
def handle_auth_callback(
    ...
    try:
        ...
        # Restore the PKCE code_verifier from the stored state so that
        # fetch_token can send it back to Google's token endpoint.
